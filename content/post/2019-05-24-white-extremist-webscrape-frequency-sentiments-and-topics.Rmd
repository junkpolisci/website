...
```{r Top Users Timeline, cache=T, echo=F, warning=F, message=F, fig.height=20, fig.width=15}
user_month %>% 
  filter(ym < "2012-01") %>% 
...
```{r Sentiment Analysis Top 10 Users NRC, echo= F, cache=T, warning=F, message=F,  fig.width= 15, fig.height=10}
unigram_nrc_user %>% 
...
```{r Unigram LDA 5 A, cache=T, echo=F, warning=F, message=F, fig.width=15, fig.height=20}
...
```{r Unigram LDA 5 B, cache=T, echo=F, warning=F, message=F, fig.width=20, fig.height=40}
u_lda5_comptops  %>% 
   ggplot(aes(x = term,
             y = beta_comp,
             fill = pair_id)) + 
